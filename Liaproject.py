...
def df_filter_function_travel(df, city_1, city_2):
    contain_values = df[df["cities"].str.contains(city_1 + "-" + city_2)]
...
def predict_menu_travel(df):
    with st.form("my_form"):
        ...
        if submitted:
            ...
            string_output = "price is " + str(math.floor(predicted_price[0])) + "tkr" + " (inklusive resekostnader)" + "fr.o.m. " + str(train_year) + "-" + str(train_month) +"-dd"
...
def result_menu(df):
    ...
    city_1 = st.sidebar.selectbox("Select which city to start from", city_list, index=city_list.index("Malmö"))
    city_2 = st.sidebar.selectbox("Select which city to end in", city_list)
    for x in df.index:
...
def main():
    ...
    option = st.sidebar.selectbox('what would you like to do', ('Append', 'Predict','Predict(travel cost included)', 'Show data', 'Show correlation', "Calculate travel distance"))
    options = {"Append" : "append_menu(df_train)", "Predict" : "predict_menu(df_train)", "Predict(travel cost included)" : "predict_menu_travel(df_train)", "Show data" : "result_menu(df_train)", "Show correlation" : "correlation_menu(df_train)", "Calculate travel distance" : "travel_cost()"}
    eval(options[option])
